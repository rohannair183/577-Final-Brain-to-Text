...
def test_per_calculation():
    """Test if PER calculation is correct"""
    
    metrics = PhonemeMetrics(phoneme_to_char_map=None)
    
    # Test 1: Perfect predictions (PER should be 0.0)
    predictions = torch.randn(2, 10, 41)  # (batch, time, classes)
    targets = torch.randint(1, 41, (2, 10))  # (batch, time)
    
    # Make predictions match targets perfectly
    for b in range(2):
        for t in range(10):
            predictions[b, t, targets[b, t]] = 10.0  # High logit
    
    target_lengths = torch.tensor([10, 10])
    result = metrics.compute_phoneme_error_rate(predictions, targets, target_lengths)
    
    print("Test 1: Perfect predictions")
    print(f"  Expected PER: 0.0")
    print(f"  Actual PER: {result:.4f}")
    
    if result > 0.01:
        print("  ❌ FAIL - PER should be 0!")
        return False
    
    # Test 2: Random predictions (PER should be ~0.97)
    predictions = torch.randn(2, 10, 41)
    targets = torch.randint(1, 41, (2, 10))
    target_lengths = torch.tensor([10, 10])
    
    result = metrics.compute_phoneme_error_rate(predictions, targets, target_lengths)
    
    print("\nTest 2: Random predictions")
    print(f"  Expected PER: ~0.95-0.98")
    print(f"  Actual PER: {result:.4f}")
    
    if result < 0.90:
        print("  ❌ FAIL - Random should be ~0.97!")
        return False
    
    # Test 3: Half correct (PER should be 0.5)
    predictions = torch.randn(2, 10, 41)
    targets = torch.randint(1, 41, (2, 10))
    
    # Make first half correct
    for b in range(2):
        for t in range(5):
            predictions[b, t, targets[b, t]] = 10.0
    
    target_lengths = torch.tensor([10, 10])
    result = metrics.compute_phoneme_error_rate(predictions, targets, target_lengths)
    
    print("\nTest 3: 50% correct")
    print(f"  Expected PER: ~0.5")
    print(f"  Actual PER: {result:.4f}")
    
    if abs(result - 0.5) > 0.1:
        print("  ❌ FAIL - Should be ~0.5!")
        return False
